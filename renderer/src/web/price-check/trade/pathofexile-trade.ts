...
export const CATEGORY_TO_TRADE_ID = new Map([
  [ItemCategory.Waystone, "waystone"],
...
export function createTradeRequest(
  ...
  if (filters.sockets && !filters.sockets.disabled) {
    propSet(
      query.filters,
      "socket_filters.filters.sockets.w",
      filters.sockets.value
    );
  }

  if (filters.waystoneTier && !filters.waystoneTier.disabled) {
    propSet(
      query.filters,
      "map_filters.filters.map_tier.min",
      filters.waystoneTier.value
    );
    propSet(
      query.filters,
...
export async function requestResults(
  ...
  if (!data) {
    ...
    const response = await Host.proxy(
      `${getTradeEndpoint()}/api/trade2/fetch/${resultIds.join(
        ","
      )}?query=${queryId}`
    );
