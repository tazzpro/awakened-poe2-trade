...
export async function requestPoeprices(
  ...
  if (data.currency === "chaos") {
    ...
    const xchgChaos = findPriceByQuery({
      ns: "ITEM",
      name: "Chaos Orb",
      variant: undefined,
    });
    if (!xchgChaos) {
      throw new Error("poeprices.info gave the price in Chaos Orbs.");
    }
    const converted = autoCurrency([
      data.min * xchgChaos.exalt,
      data.max * xchgChaos.exalt,
    ]);
    data.min = converted.min;
    data.max = converted.max;
    data.currency = converted.currency === "div" ? "divine" : "exalt";
  } else if (data.currency !== "divine" && data.currency !== "exalt") {
    throw new Error("poeprices.info gave the price in unknown currency.");
  }

  return {
    currency: data.currency === "divine" ? "div" : "exalt",
    min: data.min,
    max: data.max,
